...
const getAllProducts = asyncHandler(async(req,res) => {
    ...
    const products = await Product.find(filter).skip(skip).limit(limit).sort({[sortBy]:sortOrder});
    res.status(200).json({total,page,pages: Math.ceil(total/limit),limit,products});
})
...
const createProduct = asyncHandler(async (req,res) => {
    ...
    res.status(201).json({
        ...
        product: {name: newProduct.name, description: newProduct.description, category: newProduct.category, price:newProduct.price, stock:newProduct.stock},
    })
})

module.exports = {getAllProducts, getProductById, updateProductById, deleteProductById, uploadProductimg, createProduct};